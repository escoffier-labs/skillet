...
- **fleet-conductor** - conducts large backlog burn-downs and multi-repository campaigns: bounded lanes, collision and held-trigger tracking, draft-until-gates landing (issue linkage, truthful body, scope, actual diff, required checks, receipts, collision review, independent Codex and Opus 5 reviews), required-vs-optional check handling, and merge verification from GitHub `MERGED` with non-null `mergeCommit` and `mergedAt`. Includes the shared Untrusted content section and is listed in `EXTERNAL_CONTENT_SKILLS`. Baseline-tested against a fresh agent before it shipped.
...
- The Brigade work loop (brief -> work -> verify with capture -> handoff) is now defined once in `docs/brigade-process-model.md` as a process model with named gates. `using-skillet`, `check`, and `memory-handoff` reference it instead of restating their own versions of the loop.
