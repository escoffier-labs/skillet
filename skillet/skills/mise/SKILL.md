...
9. **Hand off to recipe.** That is the only skill you invoke next - not a frontend skill, not an implementation skill, not the code itself. (Until the `recipe` port lands, fall back to `writing-plans`.)
